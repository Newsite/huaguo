	...
	{    //验证中文  
		...
		mobileTelephone:{
			validator:function(value){
				...
				var tel = /^\d{3,4}?-[2-9]\d{6,7}$/;
				var tel2 = /^[2-9]\d{6,7}$/;
				return tel.test(value) || tel2.test(value) || (value.length == 11 && cmccMobile.test(value));
			},
			message:"请正确填写您的联系电话(手机/电话皆可，电话格式12345678或者010-12345678)."
		},
		//传真
		fax:{
			validator:function(value){
		...
		// compareDate2不起作用，所以重写
		//日期、时间验证大小，结束日期应该大于等于开始日期
		//可以自定义提示信息
		compareDateGE:{
			validator:function(toDate,param){
				if(param ==null || param.length ==0 || param[0]==null || param[0]==""){
				...
				if(toDate >= param[0]){
					...
					if(param.length >= 2){
						$.fn.validatebox.defaults.rules.compareDateGE.message = param[1];
					}
					else {
						$.fn.validatebox.defaults.rules.compareDateGE.message = '结束日期应该大于等于开始日期';
					}
					return false;
				}
			},
		...
		emailLength:{
			validator:function(_1f, _1g){
				var len=$.trim(_1f).length;
				return len>=_1g[0]&&len<=_1g[1]&&/^((([a-z]|\d|[!#\$%&'\*\+\-\/=\?\^_`{\|}~]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])+(\.([a-z]|\d|[!#\$%&'\*\+\-\/=\?\^_`{\|}~]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])+)*)|((\x22)((((\x20|\x09)*(\x0d\x0a))?(\x20|\x09)+)?(([\x01-\x08\x0b\x0c\x0e-\x1f\x7f]|\x21|[\x23-\x5b]|[\x5d-\x7e]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(\\([\x01-\x09\x0b\x0c\x0d-\x7f]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF]))))*(((\x20|\x09)*(\x0d\x0a))?(\x20|\x09)+)?(\x22)))@((([a-z]|\d|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(([a-z]|\d|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])([a-z]|\d|-|\.|_|~|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])*([a-z]|\d|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])))\.)+(([a-z]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(([a-z]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])([a-z]|\d|-|\.|_|~|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])*([a-z]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])))\.?$/i.test(_1f);
			},
			message:"请输入长度介于{0}和{1}之间有效的电子邮件地址"
		},
		// 带中文的字符串长度（中文长度为2）
		lengthCN:{
		...
		isValidaUrl:{
			...
			message:"请正确填写您的个人主页(格式http://www.demo.com)."
		}
		//到服务器端验证
		/*remote:{
