...
public class MessageDaoImpl extends GenericDaoImpl<MessageEntity, Long>
	...
	public List<MessageEntity> getAllMessageByUserId(Long userId, String isRead,String noticeOrLetter) {
		String hql = " from MessageEntity as m where m.receiver.userId = ?  and m.isRead = ? and m.type= ? and m.othersDelTag='0'";
		List<MessageEntity> list = getHibernateTemplate().find(hql,userId,isRead,noticeOrLetter);
	...
	public Integer getAllUnreadCountByUserId(Long userId) {
		String hql = "from MessageEntity as m where m.receiver.userId = ? and m.isRead = ? and m.type= ? and m.ownDelTag='0'";
		List<MessageEntity> list = getHibernateTemplate().find(hql,userId,Constants.MESSAGE_NOT_READED,Constants.MESSAGE_PRIVATELETTER);
	...
	public List<MessageEntity> getLinkManCount(Long userId,String isNoticeOrPrivateLetter) {
		String hql= "SELECT DISTINCT m.receiver.userId from MessageEntity as m where ( m.receiver.userId <> ? and m.sender.userId = ? ) and m.type= ? and m.ownDelTag='0' order by createDate desc" ;
		String hql2= "SELECT DISTINCT m.sender.userId from MessageEntity as m where ( m.receiver.userId = ? and m.sender.userId <> ? ) and m.type= ? and m.othersDelTag='0' order by createDate desc" ;
		List<MessageEntity> list = getHibernateTemplate().find(hql,userId,userId,Constants.MESSAGE_PRIVATELETTER);
		List<MessageEntity> list2 = getHibernateTemplate().find(hql2,userId,userId,Constants.MESSAGE_PRIVATELETTER);
