	...
	$(document).ready(function() {
		...
		$.post("${ctx}/merchant/getMerchant?merchantId="+merchantId,null,function(data)
		{
			...
			if(data.imageUrl!="" && data.imageUrl!=null){
				//$("#imgDiv").show();
	 			//$("#img_preview").attr('src',fileSys+data.imageUrl);
			}
 			...
 			{
 				content = "<strong>关于餐厅</strong><br>在这边你可以编写餐厅的故事，比如说创建的心路历程、餐厅名称的来由、还有对于餐厅的整体规划。<br><br><strong>关于味道</strong><br>在这边你可以来说说餐厅的菜式，特色招牌菜。可以图文并茂的介绍店内独创的特色佳肴";
 			}
 			editor.html(content);
 			var previewImageUrl = "";
 			if(null!=data.imageUrl)
 			{
 				previewImageUrl=ctx+data.imageUrl;
 			}
 			previewMerchantSmall(previewImageUrl);
 			
 			if(null!=data.linkMan)
 			{
 				$("#linkMan").val(data.linkMan);
 	 			$("#merchantTel").val(data.merchantTel);
 	 			$("#picUrl").val(data.picUrl);
 	 			$("#picDiv").show();
 	 			$("#pic_preview").attr('src',ctx+data.picUrl);
 			}
	...
	var obj={  
		...
		function changeMerchantCity()
		{
		 ...
		 {		 
		    ...
		    function previewMerchantSmall(imageUrl)
		    {
		    ...
		    {  
		    	...
		    	if($("#merchantId").val()=="")
		    	{
		    	 ...
		    	 var fields = $("#ownerForm").serializeArray();	
		 			$.post("${ctx}/merchant/saveMerchantOwner?isOpen=0",fields,function(data){			
		 				$('#popup_message').popup({'html': data.msg});
		 			changeToMerchantLog();
		 		});
		    }
		    ...
		    function verifyMerchant()
		    {
		    	...
		    	var merchantId = $("#merchantId").val();
		    	$.post("${ctx}/merchant/verifyMerchant?merchantId="+merchantId,null,function(data)
		    	{			
		 			if(data.error)
		 			{
		 				$('#popup_message').popup({'html': data.msg});
		    ...
		    function doPreview()
		    {
		    	...
		    	window.open(ctx+"/merchant/merchantPreview/"+$("#merchantId").val());
		    }
		    ...
		    {  
		    	...
		    	editor.sync();
		    	 ...
		    	 var fields = $("#contentForm").serializeArray();	
		 			$.post("${ctx}/merchant/updateContent?merchantId="+merchantId,fields,function(data){			
		 			if(data.error!=null)
		 			{
		 				if(data.error=="1")
		 				{
           ...
           <section id="contentSection" class="clearfix">
             <div class="Registerform DiningRoom support">
               ...
               <div class="interlaced determine">
                 ...
                 <button class="btnOrng" type="button" onclick="changeToMerchantDetail2()">下一步</button>
               </div>
             </div>
           </section>
           <aside id="registerRight" class="register-right" role="">
             <div class="IssuedNotice">
               ...
               <div class="previewlist">
                 <div>
                   <section id="" class="profile clearfix">
                     <div class="featured-img"> <a href="javascript:void(0);" class="thumbnail" title=""><img id="previewImage" width="302" height="183" src="${ctx}/static/images/defaultMerchant.png" class="" alt="" title=""></a>
                       <div class="Name">
         ...
         <form method="post" id="contentForm" enctype="multipart/form-data">
           <section id="contentSection" class="clearfix">
             <div class="Registerform DiningRoom support">
               ...
               <div class="interlaced determine">
                 <button class="btnOrng" type="button" onclick="saveMerchantContent()">保存</button>
                 <button class="btnOrng" type="button" onclick="doPreview()">预览餐厅</button>
                 <button class="btnOrng" type="button" onclick="changeToMerchantOwner2()">下一步</button>
               </div>
             </div>
           </section>
         ...
         <form method="post" id="ownerForm" enctype="multipart/form-data">
           <section id="content2" class="">
             <div class="Registerform DiningRoom support">
               ...
               <div class="interlaced determine">
                 <button class="btnOrng" type="button" onclick="saveMerchantOwner()">保存,下一步</button>
                 <button class="btnOrng" type="button" onclick="changeToMerchantLog()">跳过</button>
               </div>
             </div>
           </section>
         </form>
       ...
       <div id="logDiv" class="Formlist hidden">
         <section id="content2" class="clearfix">
           <div class="Registerform DiningRoom support">
             ...
             <div class="iisi"></div>
             <div class="interlaced determine"> <a class="btnOrng" title="" href="javascript:void(0);" onclick="verifyMerchant()">提交审核</a> <a class="btnOrng" title="" href="javascript:void(0);" onclick="doPreview()">餐厅预览</a> </div>
             <header class="introduce-title">
               <h1 class="introduce">维护日志</h1>
             </header>
             <div class="iisi"></div>
             <div class="interlaced editor2">
